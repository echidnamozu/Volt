    ...
    function(Backbone, Hypr) {
        ...
        StreetAddress = Backbone.MozuModel.extend({
            ...
            requiresStateAndZip: function(value, attr) {
                if((this.get('countryCode') in countriesRequiringStateAndZip) && !value){
                     return this.validation[attr.split('.').pop()].msg;
                }else{
                    if(attr === "editingContact.address.postalOrZipCode"){ 
                        if(/^[a-zA-Z0-9- ]*$/.test(value) === false) {
                            return Hypr.getLabel("postalCodeHasSpecialChars");  
                        }
            ...
            defaults: {
                ...
                countryCode: Hypr.getThemeSetting('preselectCountryCode') || 'US', 
