    ...
    var qs = '%QUERY',
        ...
        makeSuggestionGroupFilter = function(name) {
            return function(res) {
                ...
                var filtersuggestion= res.suggestionGroups[0].suggestions;
                var finalsuggestions=res.suggestionGroups;
                var tempdata=[];
                $(filtersuggestion).each(function(data,k){
                    var flagg=true;
                      $(k.suggestion.properties).each(function(datas,j){
                        if(j.attributeFQN=="tenant~hide-product"){
                            $(j.values).each(function(datas,l){
                                if(l.value){
                                   flagg=false; 
                                }else{
                ...
                if(tempdata.length >0){
                    finalsuggestions[0].suggestions=tempdata;
                }else{
                     finalsuggestions[0].suggestions=tempdata;
                }
                    var thisGroup;
                for (i = finalsuggestions.length - 1; i >= 0; i--) {
                    if (finalsuggestions[i].name === name) {
                        thisGroup = finalsuggestions[i];
                        break;
                    }
                }
